...
class MultiHeadMaskedAttention(nn.Module):
    def __init__(self, config: LLMConfig) -> None:
        ...
        self.qkv = nn.Linear(
            config.hidden_size, (config.num_heads + 2 * config.num_kv_heads) * head_size
        )
        ...
        self.register_buffer(
            "rope_cos",
            torch.cos(
                torch.arange(0, config.hidden_size // config.num_heads, 2)
    ...
    def _apply_rope(self, x: torch.Tensor, T: int, past_T: int) -> torch.Tensor:
        ...
        return torch.cat(
            [
                ...
                - x2 * self.rope_sin[:, :, past_T : past_T + T],
                x2 * self.rope_cos[:, :, past_T : past_T + T]
                + x1 * self.rope_sin[:, :, past_T : past_T + T],
            ],
            dim=-1,
        )
    ...
    def forward(
        ...
        q, k, v = self.qkv(x).split(
            [
                self.config.num_heads * head_size,
                self.config.num_kv_heads * head_size,
                self.config.num_kv_heads * head_size,
            ],
            dim=-1,
        )  # (B, T, num_heads * head_size | num_kv_heads * head_size)

        q = q.view(B, T, self.config.num_heads, head_size).transpose(
            1, 2
        )  # (B, num_heads, T, head_size)
        q = self._apply_rope(q, T, past_T)

        k = k.view(B, T, self.config.num_kv_heads, head_size).transpose(
            1, 2
        )  # (B, num_kv_heads, T, head_size)
        k = self._apply_rope(k, T, past_T)

        v = v.view(B, T, self.config.num_kv_heads, head_size).transpose(
            1, 2
        )  # (B, num_kv_heads, T, head_size)
        ...
        k = torch.repeat_interleave(
            k, self.config.num_heads // self.config.num_kv_heads, dim=1
        )  # (B, num_heads, T, head_size)
        v = torch.repeat_interleave(
            v, self.config.num_heads // self.config.num_kv_heads, dim=1
        )  # (B, num_heads, T, head_size)

        mask = self.mask[past_T : past_T + T, : past_T + T]
        if self.config.flash_attention:
            y = F.scaled_dot_product_attention(q, k, v, mask, dropout_p=0.0).view(
                B, T, C
            )
        else:
...
class LLM(nn.Module):
    @classmethod
    def from_pretrained(
        cls, path: str, map_location: str | torch.device | None = None
    ) -> "LLM":
    ...
    def __init__(self, config: LLMConfig) -> None:
        ...
        self.tok_embed: nn.Embedding = nn.Embedding(
    ...
    def forward(
        ...
        if y is not None:
            loss = F.cross_entropy(
                logits.view(-1, logits.shape[-1]), y.view(-1), ignore_index=PAD_TOKEN_ID
            )

        return logits, loss, new_kv_cache

    def generate(
        self, x: torch.Tensor, top_p: float = 0.9, use_kv_cache: bool = True
    ) -> torch.Tensor:
        _, T = x.shape
        kv_cache = None
